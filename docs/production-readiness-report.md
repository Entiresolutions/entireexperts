...
| `npm run build` (`next build`) | ✅ Succeeds — 65 routes generated (static + SSG + dynamic API routes) |
| `npm run test` (Vitest, 51 tests) | ✅ All passing |
| `npm run test:e2e` (Playwright, 15 tests) | ✅ All passing |
| `npm run start` smoke test against the production build | ✅ Spot-checked: home, service, blog, portfolio, industry, sitemap, robots, manifest, contact, quote, 404 |

## What's fully built and working

- **All required pages**: 17 main pages, 18 unique service landing pages, 6 industry pages, 5 blog posts + category pages, 4 illustrative case studies, custom 404 and error boundaries (`error.tsx` + `global-error.tsx`).
...
|---|---|---|
| Contact email, phone, address | Placeholder email (`hello@entireexpert.com`); phone and address are `null` so no unverified address is ever published in structured data | `content/company.ts` |
...
| Social media links | Placeholder URLs (`linkedin.com/company/entireexpert`, etc.) — replace with real profiles or remove | `config/site.ts` |
