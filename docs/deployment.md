...
|---|---|---|
| `SITE_URL` / `NEXT_PUBLIC_SITE_URL` | Yes | Canonical site URL used for metadata, sitemap, and OG tags. Set to `https://www.entireexpert.com` in production. |
...
6. Add the custom domain (`www.entireexpert.com`) under **Project Settings → Domains**, and set up the DNS records Vercel provides. Configure a redirect from the apex domain (`entireexpert.com`) to `www.entireexpert.com` (or vice versa — pick one canonical host and redirect the other) in Vercel's domain settings.
7. Verify the sending domain in Resend and point `CONTACT_FROM_EMAIL` at an address on that domain (e.g. `no-reply@entireexpert.com`).
...
1. In [Google Search Console](https://search.google.com/search-console), add a property for `https://www.entireexpert.com` (Domain property is preferred if you can verify via DNS; URL-prefix property works with the HTML-tag method below).
...
3. **Submit the sitemap**: in Search Console, go to Sitemaps and submit `https://www.entireexpert.com/sitemap.xml` (generated dynamically by `app/sitemap.ts` — it stays current automatically as services, blog posts, and case studies are added).
...
- [ ] `https://www.entireexpert.com/sitemap.xml` returns 200 and lists all pages.
- [ ] `https://www.entireexpert.com/robots.txt` returns 200 and references the sitemap.
...
- [ ] Confirm security headers are present (`curl -I https://www.entireexpert.com` — check for `Content-Security-Policy`, `Strict-Transport-Security`, `X-Frame-Options`).
